...
fun ChatsPage(vm: AppViewModel) {
    ...
    // Check bloom rituals when chats page is shown
    LaunchedEffect(Unit) {
        kotlinx.coroutines.delay(2000)
        vm.checkBloomRituals()
        // Check seasonal friendships — contacts with >90 days quiet
        val quietContact = vm.contacts.firstOrNull { it.days >= 90 }
        if (quietContact != null && vm.seasons.none { it.contactId == quietContact.id }) {
            vm.seasons.add(AppViewModel.FriendshipSeason(
                contactId = quietContact.id,
                contactName = quietContact.name,
                label = "Spring 2025",
                messageCount = quietContact.messages.size,
                callCount = 3,
                vibeCount = 1
            ))
            vm.currentSeason = vm.seasons.last()
            vm.showSeasonCard = true
        }
    }
...
private fun GroupChatRow(gc: com.cloes.app.data.GroupChat, vm: AppViewModel, c: CloesColors) {
    Row(modifier = Modifier.fillMaxWidth()
        ...
        // Group profile picture — tap to change
        val gcIdx = vm.groupChats.indexOf(gc)
        Box(modifier = Modifier.size(46.dp).clip(RoundedCornerShape(14.dp))
            ...
            .clickable {
                // Cycle through gradient colours as a photo-picker simulation
            ...
            if (gc.photoUri != null) {
                Box(modifier = Modifier.fillMaxSize()
                    .background(Brush.radialGradient(listOf(Purple.copy(0.5f), Pink.copy(0.4f)))),
                    contentAlignment = Alignment.Center) {
            ...
            // Small camera badge
            Box(modifier = Modifier.align(Alignment.BottomEnd).size(15.dp)
                .clip(androidx.compose.foundation.shape.CircleShape)
                .background(Purple.copy(0.85f)).border(1.dp, c.bg, androidx.compose.foundation.shape.CircleShape),
                contentAlignment = Alignment.Center) {
                Icon(Icons.Default.CameraAlt, null, tint = Color.White, modifier = Modifier.size(8.dp))
            }
        }
        ...
        Row(modifier = Modifier.fillMaxWidth()
            ...
            .combinedClickable(
                onClick = {
                    ...
                    if (contact.locked && !vm.showLockedContacts) vm.showToast("Locked", "Type your CLOESED KEY in search to reveal", "mid")
