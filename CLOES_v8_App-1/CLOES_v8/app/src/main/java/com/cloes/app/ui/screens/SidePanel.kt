...
fun SidePanel(vm: AppViewModel) {
    ...
    Box(
        modifier = Modifier.fillMaxHeight().width(290.dp).shadow(24.dp)
            .background(if (vm.appTheme == AppTheme.Default || vm.appTheme == AppTheme.Dark) Color(0xFF12092B) else Color(0xFF1A0F2E))
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            // Header
            Box(
                modifier = Modifier.fillMaxWidth().background(Color(0xFF1E1040))
                ...
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    Text(vm.profile.name, color = Color.White, fontSize = 16.sp, fontWeight = FontWeight.Bold)
            ...
            HorizontalDivider(color = Color(0xFF2D1F5C), thickness = 0.5.dp)
...
private fun SidePanelMenuItem(item: PanelItem, c: CloesColors, onClick: () -> Unit) {
    Row(
        ...
        Column(modifier = Modifier.weight(1f)) {
            ...
            Text(item.subtitle, color = Color(0xFF7A6A9A), fontSize = 11.sp)
        }
        Icon(Icons.Default.ChevronRight, null, tint = Color(0xFF7A6A9A), modifier = Modifier.size(16.dp))
    }
}
