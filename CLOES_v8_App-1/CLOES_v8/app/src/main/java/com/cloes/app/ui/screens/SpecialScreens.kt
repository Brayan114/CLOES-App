        ...
        Column(
            ...
            Box(
                modifier = Modifier
                    ...
                    .background(Color(0xFF0EA5E9).copy(alpha = 0.18f))
                    .border(1.dp, Color(0xFF0EA5E9).copy(alpha = 0.6f), RoundedCornerShape(14.dp))
                    .clickable {
                        val shareText = "🚨 EMERGENCY — I need help! My last known location:\nhttps://maps.google.com/?q=my+location\n\nSent via CLOES Emergency Mode"
                        val intent = Intent(Intent.ACTION_SEND).apply {
                            type = "text/plain"
                            putExtra(Intent.EXTRA_TEXT, shareText)
                            putExtra(Intent.EXTRA_SUBJECT, "🚨 Emergency Location Share")
                        }
                        context.startActivity(Intent.createChooser(intent, "Share Location With..."))
                    }
                ...
                Row(
                    ...
                    Icon(Icons.Default.LocationOn, null, tint = Color(0xFF0EA5E9), modifier = Modifier.size(22.dp))
                    Column(modifier = Modifier.weight(1f)) {
                        Text("Share My Location", color = Color.White, fontSize = 14.sp, fontWeight = FontWeight.SemiBold)
                        Text("Send location to emergency contacts", color = Color.White.copy(alpha = 0.65f), fontSize = 11.sp)
                    }
                    Icon(Icons.Default.ChevronRight, null, tint = Color(0xFF0EA5E9), modifier = Modifier.size(18.dp))
                }
            }
            ...
            Column(
                ...
                Text("EMERGENCY CONTACTS", color = Color.White.copy(0.85f), fontSize = 9.sp,
                ...
                vm.profile.emergencyContacts.forEach { ec ->
                    Row(
                        ...
                        Column {
                            ...
                            Text(ec.type, color = Color.White.copy(0.85f), fontSize = 11.sp)
                        }
                        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                            // Share location with this specific contact
                            Box(
                                modifier = Modifier.size(36.dp).clip(CircleShape)
                                    .background(Color(0xFF0EA5E9).copy(0.2f))
                                    .border(1.dp, Color(0xFF0EA5E9), CircleShape)
                                    .clickable {
                                        val shareText = "🚨 EMERGENCY — I need help! My last known location:\nhttps://maps.google.com/?q=my+location"
                                        val intent = Intent(Intent.ACTION_SEND).apply {
                                            type = "text/plain"
                                        ...
                                        context.startActivity(Intent.createChooser(intent, "Share location with ${ec.name}"))
                                    },
                                contentAlignment = Alignment.Center
                            ) {
                                Icon(Icons.Default.LocationOn, null, tint = Color(0xFF0EA5E9),
    ...
    Box(modifier = Modifier.fillMaxSize().background(c.bg).imePadding()) {
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            Row(modifier = Modifier.fillMaxWidth().background(c.surface2)
                ...
                val gcIdx = vm.groupChats.indexOfFirst { it.id == gc.id }
                Box(modifier = Modifier.size(38.dp).clip(RoundedCornerShape(11.dp))
                    .background(
                        if (gc.photoUri != null) Brush.radialGradient(listOf(Purple.copy(0.5f), Pink.copy(0.4f)))
                        else Brush.linearGradient(listOf(Purple.copy(0.3f), Pink.copy(0.3f)))
                    )
                    .clickable {
                        val samplePhotos = listOf(null, "gc_sample_1", "gc_sample_2")
                        val next = (samplePhotos.indexOf(gc.photoUri) + 1) % samplePhotos.size
                        if (gcIdx >= 0) vm.groupChats[gcIdx] = vm.groupChats[gcIdx].copy(photoUri = samplePhotos[next])
                        vm.showToast(gc.name, if (samplePhotos[next] == null) "Photo removed" else "Group photo updated ✦", "low")
                    },
                    contentAlignment = Alignment.Center) {
                    Icon(Icons.Default.Group, null,
                        tint = if (gc.photoUri != null) Color.White else Purple,
