...
private fun Step4Done(vm: AppViewModel, c: CloesColors) {
    ...
    Column(
        ...
        Spacer(modifier = Modifier.height(24.dp))
        Text("You're in, ${vm.onboardName} ✦", color = c.text, fontSize = 24.sp, fontWeight = FontWeight.Bold, textAlign = TextAlign.Center)
        Spacer(modifier = Modifier.height(8.dp))
        Text("@${vm.onboardHandle}", color = Purple, fontSize = 14.sp)
        Spacer(modifier = Modifier.height(32.dp))

        if (vm.isAuthLoading) {
            androidx.compose.material3.CircularProgressIndicator(color = Purple, modifier = Modifier.size(28.dp))
            Spacer(modifier = Modifier.height(12.dp))
            Text("Saving your profile…", color = c.textSub, fontSize = 12.sp)
        } else {
            GradientButton(
                text = "Enter CLOES →",
                onClick = { vm.doCompleteOnboard(ctx) },
                modifier = Modifier.fillMaxWidth()
            )
        }
    }
