...
class AppViewModel : ViewModel() {
    ...
    fun checkSavedSession(ctx: Context) {
        ...
        if (saved == null) { hasCheckedSession = true; return }
        isAuthLoading = true
        viewModelScope.launch {
            val result = CloesApi.fetchMe(saved)
            isAuthLoading = false
    ...
    fun doRegister(ctx: Context, name: String, email: String, password: String) {
        isAuthLoading = true; authError = null
        viewModelScope.launch {
            val result = CloesApi.register(name, email, password)
            isAuthLoading = false
            if (result.success) {
                authToken = result.token
                CloesApi.saveToken(ctx, result.token!!)
    ...
    fun doLogin(ctx: Context, email: String, password: String) {
        isAuthLoading = true; authError = null
        viewModelScope.launch {
            val result = CloesApi.login(email, password)
            isAuthLoading = false
            if (result.success) {
                authToken = result.token
                CloesApi.saveToken(ctx, result.token!!)
    ...
    fun doGoogleAuthSuccess(ctx: Context, token: String, onboarded: Boolean) {
        authToken = token
        CloesApi.saveToken(ctx, token)
        isAuthLoading = true
        viewModelScope.launch {
            val result = CloesApi.fetchMe(token)
            isAuthLoading = false
            if (result.success) {
                profile = profile.copy(name = result.userName ?: "", handle = result.userHandle ?: "")
            }
            currentScreen = if (onboarded) "main" else "onboard"
        }
    }

    fun doCompleteOnboard(ctx: Context) {
        val token = authToken ?: return
        isAuthLoading = true
        viewModelScope.launch {
            val hexColors = onboardPalette.map {
                val argb = it.hashCode()
                String.format("#%06X", 0xFFFFFF and argb)
            }
            val result = CloesApi.completeOnboard(
                token, onboardName, onboardHandle, hexColors
            )
            isAuthLoading = false
            if (result.success) {
                profile = profile.copy(
                    ...
                    palette = onboardPalette.toList()
                )
                currentScreen = "main"
            } else {
                showToast("Error", result.error ?: "Could not save profile", "mid")
            }
        }
    }
    ...
    fun addContact() {
        if (newContactHandle.isBlank()) { showToast("Add Contact", "Please enter a handle", "mid"); return }
        showAddContact = false
        val t = authToken
        if (t == null) { showToast("Error", "Not signed in", "mid"); return }
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                if (match != null) {
                    ...
                    withContext(Dispatchers.Main) {
                        showToast("Search", "Handle @$newContactHandle not found", "mid")
                        showAddContact = true
                    }
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    showToast("Error", "Network error", "mid")
                    showAddContact = true
                }
            }
        }
    ...
    fun deleteAccount(ctx: Context? = null) {
